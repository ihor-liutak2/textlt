...
void TextltApp::InitializeCommands() {
    ...
    add("sidebar.ctrl_b_file_explorer", "Sidebar", "Options", "Ctrl+B", [this] { OpenSidebarShortcutModal(); });
    add("sidebar.show_project", "Project Sidebar", "Options", "", [this] { ShowProjectSidebar(); });
...
void TextltApp::InitializeMenuShortcuts() {
    for (const AppCommand& command : command_registry_.Commands()) {
        ...
        if (!command.shortcut.empty()) {
            ...
            if (!ParseShortcutKey(first_shortcut) && first_shortcut.find(',') == std::string::npos) {
