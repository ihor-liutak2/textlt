...
LayoutController::LayoutController(DocumentWorkspace& workspace)
...
void LayoutController::SetMode(EditorLayoutMode mode) {
    mode_ = mode;
    workspace_.ClampActiveEditorPaneIndex(VisiblePaneCount());
    workspace_.EnsureEditorPanesHaveSessions(VisiblePaneCount());
}
...
ViewLayoutSnapshot LayoutController::Snapshot() const {
    ...
    snapshot.layout_name = ModeLabel();
