...
std::vector<ShortcutBindingDefinition> EditorKeymap::DefaultBindings() {
    return {
        ...
        {ShortcutContext::Text, "editor.select_paragraph_up", "Select previous paragraph", "Selection", "Ctrl+Shift+Up"},
        {ShortcutContext::Text, "editor.select_paragraph_down", "Select next paragraph", "Selection", "Ctrl+Shift+Down"},
...
bool EditorKeymap::RunAction(TextltApp& app, const std::string& action_id) const {
    ...
    if (action_id == "editor.select_paragraph_up") {
        MoveCursor(*editor, MoveAction::ParagraphUp, true);
        FinishEditorAction(app);
        return true;
    }
    if (action_id == "editor.select_paragraph_down") {
        MoveCursor(*editor, MoveAction::ParagraphDown, true);
        FinishEditorAction(app);
        return true;
    }
